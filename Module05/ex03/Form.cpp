...
void Form::execute(const Bureaucrat &executor) const {
	try {
		if (!itSigned_)
			throw NotSigned();
		else if (gradeToExecute_ < executor.getGrade())
			throw GradeTooHighException();
		else
			exec();
	}
	catch (GradeTooHighException& e){
		std::cout << executor.getName() << " cannot execute " << name_ << " because grade of form too high" << std::endl;
	}
	catch (NotSigned& e) {
		std::cout << executor.getName() << " cannot execute " << name_ << " because not signed" << std::endl;
	}
}
